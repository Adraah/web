...
const Home = ({ onLogout }) => {
  ...
  const fetchData = async () => {
    try {
      const ticketResponse = await makeApiCall('GET', 'https://9ffoua37l6.execute-api.us-east-2.amazonaws.com/getTickets');
      const infoResponse = await makeApiCall('GET', 'https://mpz96ml60e.execute-api.us-east-2.amazonaws.com/getAllInfo');

      if (!ticketResponse) return;

      const allClients = infoResponse.clients || [];
      setClients(['Todos', ...allClients.map(client => client.client)]);

      const filteredData = selectedClient === 'Todos'
        ? ticketResponse
        : ticketResponse.filter(ticket => ticket.clientName?.trim() === selectedClient);

      setTickets(mapRepairData(filteredData));
      getTimeElapsedByDay(filteredData);
      setDieWorkers(mapDieWorkerTimeData(filteredData));
      setPressChartData(mapPressCodeChartData(filteredData, infoResponse.press));
    } catch (error) {
      console.error(error);
    }
  };
          ...
          !tickets ? (
            ...
            <>
              <div style={{ marginTop: '20px', marginLeft: '5%' }}>
                <label htmlFor="clientFilter" style={{ fontSize: '18px', fontWeight: 600, color: '#132246' }}>Filtrar por cliente: </label>
                <select
                  id="clientFilter"
                  value={selectedClient}
                  onChange={(e) => setSelectedClient(e.target.value)}
                  style={{ marginLeft: '10px', padding: '6px 12px', fontSize: '16px', borderRadius: '8px' }}
                >
